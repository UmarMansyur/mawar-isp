<script lang="ts">
  ...
  let paginatedSecrets = $derived(
    filteredSecrets.slice((currentPage - 1) * perPage, currentPage * perPage),
  );
...
<svelte:head>
  <title>PPP Secrets - {data.mikrotik.name}</title>
</svelte:head>

<div class="container mx-auto px-4 py-8">
  <div class="flex items-center gap-4 mb-8">
    <a
      href="/admin/mikrotik/{data.mikrotik.id}/profiles"
    ...
    >
      ...
      <div>
        ...
        <p class="text-sm text-emerald-600">
          {syncCount} PPP Secrets berhasil disinkronkan dari Mikrotik.
        </p>
      </div>
    </div>
  {/if}
  ...
  <div class="flex flex-wrap items-center gap-4 mb-6">
    ...
    <div class="flex items-center gap-2">
      ...
      <select
        class="px-3 py-2 rounded-lg border border-slate-300 bg-white focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none text-sm"
        bind:value={perPage}
  ...
  {:else if isSyncing}
    <div class="flex flex-col items-center justify-center py-16 text-blue-600">
      <div class="relative">
        <div
          class="w-16 h-16 rounded-full border-4 border-blue-100 border-t-blue-500 animate-spin"
        ></div>
  ...
  {:else if filteredSecrets.length === 0}
    <div class="bg-white rounded-2xl p-12 text-center border border-slate-200">
      ...
      <p class="text-sm text-slate-500 mb-4">
        {searchQuery
          ? `Tidak ada user dengan nama "${searchQuery}"`
    ...
    <div class="bg-white rounded-2xl border border-slate-200 overflow-hidden">
      ...
      >
        <p class="text-sm text-slate-600">
          Menampilkan <span class="font-semibold text-slate-900"
            >{(currentPage - 1) * perPage + 1}</span
          >
          -
          <span class="font-semibold text-slate-900"
            >{Math.min(currentPage * perPage, filteredSecrets.length)}</span
          >
          dari
          <span class="font-semibold text-slate-900"
            >{filteredSecrets.length}</span
          > users
        </p>
      </div>
            ...
            >
              <td class="px-6 py-4">
                <div class="flex items-center gap-3">
                  <div
                    class="w-8 h-8 rounded-full bg-gradient-to-br {secret.disabled
                      ? 'from-slate-400 to-slate-500'
                      : 'from-emerald-500 to-teal-600'} flex items-center justify-center text-white text-xs font-medium"
                  >
                    {secret.name.substring(0, 2).toUpperCase()}
                  </div>
              ...
              <td class="px-6 py-4 text-slate-600"
                >{secret.remote_address || secret.local_address || "-"}</td
              >
              ...
              <td class="px-6 py-4 text-center">
                <button
                  class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors disabled:opacity-50 {secret.disabled
                    ? 'bg-emerald-100 text-emerald-700 hover:bg-emerald-200'
                    : 'bg-red-100 text-red-700 hover:bg-red-200'}"
                  onclick={() => toggleUserStatus(secret)}
                  disabled={togglingUser === secret.name}
                >
      ...
      {#if totalPages > 1}
        ...
        >
          ...
          <div class="flex items-center gap-1">
            <button
              class="p-2 rounded-lg hover:bg-slate-200 disabled:opacity-50 disabled:hover:bg-transparent transition-colors"
            ...
            {#each getPageNumbers() as page (page)}
              <button
                class="w-9 h-9 rounded-lg text-sm font-medium transition-colors {currentPage ===
                page
